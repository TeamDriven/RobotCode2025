...
public class Drive extends SubsystemBase {
    ...
    public void periodic() {
            ...
            if (lastPositions != null) {
                ...
                for (int j = 0; j < modules.length; j++) {
                    double velocity = (wheelPositions.positions[j].distanceMeters
                            ...
                            / dt;
                    double omega = wheelPositions.positions[j].angle.minus(lastPositions.positions[j].angle)
                            .getRadians()
                            / dt;
        ...
        if (currentDriveMode != DriveMode.CHARACTERIZATION && !modulesOrienting) {
            ...
            for (int i = 0; i < modules.length; i++) {
                ...
                // SwerveModuleState.optimize(currentSetpoint.moduleStates()[i],
                // modules[i].getAngle());
    ...
    public boolean isAutoAlignGoalCompleted() {
        boolean autoAlignComplete = autoAlignController == null || autoAlignController.atGoal();
        Logger.recordOutput("Drive/AutoAlignCOmpleted", autoAlignComplete);
